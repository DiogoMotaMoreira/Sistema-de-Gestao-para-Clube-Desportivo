...
## Módulo Treinador
- **Funcional:** Funcionalidades ligadas à API (`getEquipas()`, `getPlantel()`). O Semáforo Clínico Real (RF-16) está implementado e ligado ao backend real (Spring Boot), exibindo a prontidão dos atletas em tempo real através de semáforos visuais (`VERDE`, `AMARELO`, `VERMELHO`, `BLOQUEADO`) calculados dinamicamente com base nas ocorrências ativas da base de dados e no estado de elegibilidade do EMD. As ações de listagem e criação de Eventos/Sessões de treino, bem como transações vitais (`submeterChamada`, `submeterAvaliacao`, `guardarConvocatoria`) estão agora conectadas via Axios aos novos endpoints (`/api/v1/treinador/*`). O Treinador consome e produz dados reais, abandonando a dependência de Mocks locais para os fluxos principais. Adicionalmente, as listagens agregam agora com sucesso Sessões e Eventos via paralelismo de promessas, com as devidas permissões transversais (RBAC) ajustadas para Treinador e Direção Técnica.
